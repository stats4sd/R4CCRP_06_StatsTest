...
*In the code box below write the code below to match embedded output, to show the means and standard deviations of `LANDAREA` by `GENDERHH`, and the number of observations in each group*
...
In this case we would conclude, from the p-value of 0.296 that there is not enough evidence to say that there is a significant difference in the mean values of land area based on gender of head of household.
...
```
With the paired t-test we also using the same `t.test` function, but with a slightly different way of specifying the inputs. 
...
It is also not possible to pipe into the paired t-test since none of the arguments are the `data` - we need the two variable names.

Again, we may be concerned about the normality assumption. With the paired t-test what we care about is not the distribution of he individual variables but the distribution of the differences within each farmer. So we can use `mutate` to calculate a column of differences and then look at a histogram.
...
We may be concerned that results are being skewed by the two very high values, and again we don't have a very large data set so we cannot quite rely on the central limit theorem. So perhaps we could also try the Wilcoxan test again. And, luckily this works exactly like before following a very similar pattern. The `paired` code for `wilcox.test` is the same as the `paired` code for `t.test`, all we need to do is change the function.
...
*See if you can replicate the plot below, or choose to produce a different graph which shows the relationship between `Education` and `ApproveTrump`. Make sure to modify the scale on the x axis as well to make it more readable!*
```{r ggbar0,echo=FALSE}
